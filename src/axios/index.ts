...
export default class myaxios{
    ...
    postWithToken(url:string, data?:any, auth?:AxiosBasicCredentials) {
        let token = {}
        if (process.env.NODE_ENV === 'development') {
            //开发环境 do something
            axios.defaults.baseURL = this.baseURL;
            token = {
                'access-token':`eyJhbGciOiJIUzI1NiJ9.eyJqdGkiOiI1NjYwNDVjYmRkNjIxZGZmOGMwZjZiYzEiLCJleHAiOjE2MDM4NjYzNzd9.9bM7NLcCb4T8NSBo6n5ut-4K5UbC31X-HhaddCTd0Fc`
            }
        }else{
            axios.defaults.baseURL = this.proUrl;
        }
    ...
    deleteWithToken(url:string, data?:any) {
        let token = {}
        if (process.env.NODE_ENV === 'development') {
            //开发环境 do something
            axios.defaults.baseURL = this.baseURL;
