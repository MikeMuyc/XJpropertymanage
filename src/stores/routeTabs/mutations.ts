...
export function clearAllTabs(
    ...
    let thisRouterObj = null;
    if(state.tabList.find(item => item.routeName === state.activeTabName)){
        thisRouterObj = JSON.parse(JSON.stringify(state.tabList.find(item => item.routeName === state.activeTabName)))
        state.tabList = [];
        state.partB = [];
        state.tabList.push(thisRouterObj)
    }
}
