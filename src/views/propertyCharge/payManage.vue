                ...
                <div class="information" ref="information">
                    ...
                    <div class="infoShow">
                        ...
                        <div class="info">
                            ...
                            <Input style="width: 140px" v-model="accountInfo.clientName" v-else></Input>
                        </div>
                        <div class="info">
                            <div class="headline">账户余额：</div>
                            <div class="contentInfo" style="color: #3a7ef3;">
                                <i>¥</i>
                                <i style="font-size: 20px;padding-left: 6px">{{accountInfo.balance}}</i>
                            </div>
                           <!-- <Input style="width: 140px" type="2fixed" v-model="accountInfo.balance" v-else></Input>-->
                        </div>
                        <div class="info">
                ...
                <el-table
                    ...
                    >
                        <template slot-scope="{row}">
                            <div v-if="row.feeOperationTypeCode === '2'" style="color: #ff2c2c">-{{row.paidMoney | rounding}}</div>
                    ...
                    <el-table-column
                            prop="feeOperationTypeCode"
                            label="操作类型"
                            min-width="80"
                    >
                        <template slot-scope="{row}">
                            {{doTypeRename(row.feeOperationTypeCode)}}
                        </template>
                    </el-table-column>
                    <el-table-column
                            prop="payTypeCode"
        ...
        >
            ...
            <div class="MuModalContent" >
                <div class="aline">
                    ...
                    <div class="info">
                        <Input  placeholder="房间代码" disabled v-model="curObject.id"></Input>
                    </div>
                </div>
                <!--<div class="aline">
                ...
                <div class="aline">
                    <div class="label">预交金额</div>
                    <div class="info">
                        <Input  placeholder="请输入" v-model="addPayWater.paidMoney"></Input>
                    </div>
                </div>
                <div class="aline">
            ...
            <div slot="footer">
                <div class="pmbtn primary" style="margin-right: 10px" @click="addPay">确定</div>
...
<script lang="ts">
    ...
    export default class payWater extends Vue {
        ...
        mounted(): void {
            this.getSelection(`payType`,'支付方式')
            this.getSelection(`doType`,'预收款操作类型')
        }
        inited(curObj:any){
            this.curObject = curObj;
            this.getPayAccount(curObj.title);
            setTimeout(()=>{
                this.setPagesize();
                this.resetInfo();
            },200)
        ...
        async saveInfo(){
            if (this.subFlag) {
                ...
                this.accountInfo.balance = Number(this.accountInfo.balance)
                try{
                    await api.updatePayAccountInfo(this.accountInfo)
                    this.getPayAccount()
        ...
        async getPayList(){
            ...
            try {
                let {data} = await api.getPayList({
                    ...
                    startPaidMoney: this.startPaidMoney,
                    endPaidMoney: this.endPaidMoney,
                    // feeOperationTypeCode:this.doTypeCode,
                })
        ...
        async getPayAccount(name?:string){
            try {
        ...
        async addPay(){
            ...
            if(this.addPayWater.paidMoney===''){
                ...
                this.flag = false
                /*if(this.addPayWater.payTypeCode === '4'){
                    this.addPayWater.feeOperationTypeCode = '2'
                }
                else{
                    this.addPayWater.feeOperationTypeCode = '1'
                }*/
                try {
                    await api.addPayMoney(this.addPayWater)
        ...
        async getSelection(Array: string, name: string) {
            let {data} = await api.getInfoByName(name);
            if(name === '预收款操作类型'){
                this[`${Array}`] = [{
                    name: '全部类型',
                    code: ''
        ...
        doTypeRename(code:string){
            let obj:any =  this.doType.find(item => item.code === code)
            return obj ? obj.name : `-`
        }
    }
</script>
